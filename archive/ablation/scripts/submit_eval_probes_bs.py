#!/usr/bin/env python3
"""
Submit linear probe evaluation jobs for all batch_size ablation training runs.

Standard d=128 architecture for all 4 models.
batch_size in {512, 1024, 2048} × 5 seeds = 60 runs.

Probe output: inside each training run at {run}/probe_evaluation/

Usage:
    python3 scripts/ablation/submit_eval_probes_bs.py [--dry-run]
    python3 scripts/ablation/submit_eval_probes_bs.py --models vcreg aug_supcon
    python3 scripts/ablation/submit_eval_probes_bs.py --batch-sizes 512 1024 --force
"""
...
EOS_TRAIN_BASE = "/eos/user/d/dgenoves/anomaly_pipeline/ablation/bs/logs/train/runs"
PROJECT_DIR    = Path(__file__).resolve().parents[2]
LOG_DIR        = PROJECT_DIR / "logs/condor_logs/ablation/eval_probes_bs"
...
def get_run_info(run_dir: Path) -> Optional[dict]:
    ...
    try:
        ...
        if not run_name_m:
            return None
        m = re.match(r"(\w+)_bs_(\d+)_seed(\d+)", run_name_m.group(1))
        if not m:
            return None
...
def make_sub(run_info: dict, experiment: str, ckpt_path: str, dry_run: bool, flavour: str) -> None:
    job_name = f"{run_info['model_short']}_bs{run_info['batch_size']}_seed{run_info['seed']}"
    output_dir = str(run_info["run_dir"])

    LOG_DIR.mkdir(parents=True, exist_ok=True)
    SUB_DIR.mkdir(parents=True, exist_ok=True)

    sub_path = SUB_DIR / f"eval_probe_bs_{job_name}.sub"
...
def main():
    ...
    filter_bs     = set(args.batch_sizes) if args.batch_sizes else None
    ...
    if filter_bs:
        all_runs = [r for r in all_runs if r["batch_size"] in filter_bs]
    if filter_seeds:
        all_runs = [r for r in all_runs if r["seed"] in filter_seeds]

    best: dict[tuple, dict] = {}
    for r in all_runs:
        key = (r["model"], r["batch_size"], r["seed"])
    ...
    for key in sorted(best):
        r = best[key]
        experiment = EXPERIMENT_MAP.get((r["model_short"], r["batch_size"]))
        if experiment is None:
            print(f"  [WARN] No experiment config for {r['model']} bs={r['batch_size']}")
            missing += 1
            continue

        if already_done(r["run_dir"]) and not args.force:
            print(f"  [SKIP] {r['model']} bs={r['batch_size']} seed={r['seed']} — already done ({r['run_dir'].name})")
            skipped += 1
            continue
