#!/usr/bin/env python3
"""
Submit linear probe evaluation jobs for all weight_decay ablation training runs.

Standard d=128 architecture for all 4 models.
weight_decay in {0, 1e-5, 1e-3, 1e-2} × 5 seeds = 80 runs.

Probe output: inside each training run at {run}/probe_evaluation/

Usage:
    python3 scripts/ablation/submit_eval_probes_wd.py [--dry-run]
    python3 scripts/ablation/submit_eval_probes_wd.py --models vcreg aug_supcon
    python3 scripts/ablation/submit_eval_probes_wd.py --wds 0 1e3 --force
"""
...
EOS_TRAIN_BASE = "/eos/user/d/dgenoves/anomaly_pipeline/ablation/wd/logs/train/runs"
PROJECT_DIR    = Path(__file__).resolve().parents[2]
LOG_DIR        = PROJECT_DIR / "logs/condor_logs/ablation/eval_probes_wd"
...
def get_run_info(run_dir: Path) -> Optional[dict]:
    ...
    try:
        ...
        if not run_name_m:
            return None
        m = re.match(r"(\w+)_wd_(\w+)_seed(\d+)", run_name_m.group(1))
        if not m:
            return None
...
def make_sub(run_info: dict, experiment: str, ckpt_path: str, dry_run: bool) -> None:
    job_name = f"{run_info['model_short']}_wd{run_info['wd_slug']}_seed{run_info['seed']}"
    output_dir = str(run_info["run_dir"])

    LOG_DIR.mkdir(parents=True, exist_ok=True)
    SUB_DIR.mkdir(parents=True, exist_ok=True)

    sub_path = SUB_DIR / f"eval_probe_wd_{job_name}.sub"
...
def main():
    ...
    filter_wds    = set(args.wds) if args.wds else None
    ...
    if filter_wds:
        all_runs = [r for r in all_runs if r["wd_slug"] in filter_wds]
    if filter_seeds:
        all_runs = [r for r in all_runs if r["seed"] in filter_seeds]

    best: dict[tuple, dict] = {}
    for r in all_runs:
        key = (r["model"], r["wd_slug"], r["seed"])
    ...
    for key in sorted(best):
        r = best[key]
        experiment = EXPERIMENT_MAP.get((r["model_short"], r["wd_slug"]))
        if experiment is None:
            print(f"  [WARN] No experiment config for {r['model']} wd={r['wd_slug']}")
            missing += 1
            continue

        if already_done(r["run_dir"]) and not args.force:
            print(f"  [SKIP] {r['model']} wd={r['wd_slug']} seed={r['seed']} — already done ({r['run_dir'].name})")
            skipped += 1
            continue
