#!/bin/bash
# =====================================================
#  HTCondor wrapper: VCReg encoder training (10 seeds) + AE (QCD-only)
#  for a given d_model variant of 12-class nosparse
#
#  Phase 1: train VCReg encoder from scratch for each seed
#  Phase 2: run AE anomaly detection (QCD-only normal class)
#
#  Environment variables (set in .sub file):
#    PHASE1_EXPERIMENT  - Hydra encoder training config
#    PHASE2_EXPERIMENT  - Hydra AE config
...
echo "[`date`] Starting VCReg encoder+AE job on $(hostname)"
...
PHASE1_EXPERIMENT="${PHASE1_EXPERIMENT:-vcreg_12class_nosparse_dmodel64_cern}"
PHASE2_EXPERIMENT="${PHASE2_EXPERIMENT:-anomaly_qcd_vs_higgs_embedding_vcreg_nosparse_dmodel64_cern}"
ENCODER_SEEDS_DIR="${ENCODER_SEEDS_DIR:-/eos/user/d/dgenoves/anomaly_pipeline/encoder_seeds/${PHASE1_EXPERIMENT}}"
OUTPUT_DIR="${OUTPUT_DIR:-/eos/user/d/dgenoves/anomaly_pipeline/strategies_v2/${PHASE1_EXPERIMENT}_seeds}"
N_SEEDS="${N_SEEDS:-10}"
...
for SEED in $(seq 0 $((N_SEEDS - 1))); do
    ...
    # Phase 2: AE anomaly detection (QCD-only)
