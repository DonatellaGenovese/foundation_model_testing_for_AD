...
#  ALL strategies (mse_qcd, auroc_all_signals, auroc_cls12/13/14)
#  Threshold and AUROC computed on QCD only (--score-classes 0)
# =====================================================

set -euo pipefail

echo "[`date`] Starting raw AE allSM+QCDthresh (all strategies) job on $(hostname)"
...
BASE_DIR=/eos/user/d/dgenoves/anomaly_pipeline/strategies_v2/nosparse_dmodel128_seeds/raw_allsm_allstrategies_qcdthresh
...
for SEED in $(seq 0 $((N_SEEDS - 1))); do
    ...
    PYTHON_CMD="python scripts/run_anomaly_strategies.py \
        ...
        --seed ${SEED} \
        --output-dir ${SEED_OUT} \
        --model-tag raw_allsm_allstrategies_qcdthresh \
        --score-classes 0"
