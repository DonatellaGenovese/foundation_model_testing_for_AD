...
class SupConLoss(nn.Module):
    ...
    def forward(self, features: torch.Tensor, labels: torch.Tensor, strict_multi_class: bool = True, debug: bool = False) -> torch.Tensor:
        ...
        # Compute log probability for each positive pair (do the difference of log)
...
class COLLIDE2VAugmentedSupConLitModule(LightningModule):
    ...
    def __init__(
        ...
        # Cache validation embeddings/labels to compute epoch-level silhouette score.
        self._val_emb_cache = []
    ...
    def validation_step(
        ...
        """
        x, labels = batch
        # Validation contrastive proxy should use one view per sample.
        outputs = self.model_step(batch, batch_idx=batch_idx, num_views=1)
        ...
        # Cache validation embeddings for silhouette score at epoch end.
    ...
    def test_step(
        ...
        """
        x, labels = batch
        # Keep test contrastive proxy aligned with validation behavior.
        outputs = self.model_step(batch, batch_idx=batch_idx, num_views=1)
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        Updates best validation accuracy and logs silhouette score.
        """
    ...
    def _build_model_components(
        ...
        if self.hparams.use_classification_head:
            ...
            # Keep accuracy metrics aligned with the current task cardinality.
    ...
    def on_load_checkpoint(self, checkpoint: Dict[str, Any]) -> None:
        ...
        print(f"   • Augmentation: mask_p={self.hparams.mask_probability}")
